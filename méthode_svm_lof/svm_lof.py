...
def testeval (clf, clfcap, X_test, X_testcap, brouilvrai, brouilvraicap) :
    ...
    if brouildetec == brouilvrai:
        scoretrame = 1
    else:
        scoretrame = 0
    if brouildetec and not brouilvrai:
        faussedetec = 1
    else:
        faussedetec = 0
    ...
    # else:
    #     scoretrame = 1
    # if (brouildetec and not brouilvrai) or (brouildeteccap and not brouilvraicap):
    #     faussedetec = 1
    # else:
    #     faussedetec = 0
    #
...
train_dataRMC = open('/home/guillaume/PFE/DATA/dataset_RMC.json')
...
for kernel in listetypekernel:
    ...
    for i in range(1,100):
        for j in range(1,10):
            print(i)
            clf = svm.OneClassSVM(nu=0.001*i, kernel=kernel, gamma=0.1*j)
            clf.fit(X_train)
            clfcap = svm.OneClassSVM(nu=0.007, kernel="rbf", gamma=0.5)
            clfcap.fit(X_traincap)
            evaluateur.append([clf, clfcap])
            nomevaluateur.append("SVM distance type : " + kernel + " nu = " + str(round(i,1)) + " gamma = " + str(round(j)))



# clf = svm.OneClassSVM(nu=0.007, kernel="rbf", gamma=0.3)
# clf.fit(X_train)
# clfcap = svm.OneClassSVM(nu=0.007, kernel="rbf", gamma=0.5)
# clfcap.fit(X_traincap)
# evaluateur.append([clf,clfcap])
# nomevaluateur.append("SVM")
#
# clf1 = LocalOutlierFactor(n_neighbors=20, novelty=True, contamination=0.1)
# clf1.fit(X_train)
# clfcap1 = LocalOutlierFactor(n_neighbors=20, novelty=True, contamination=0.1)
# clfcap1.fit(X_traincap)
# evaluateur.append([clf1,clfcap1])
# nomevaluateur.append("LOF distance n =" + str(i+10))

for i in range(20):
    for j in range(1,50):
        clf1 = LocalOutlierFactor(n_neighbors=i+10, novelty=True, contamination=0.01*j)
        clf1.fit(X_train)
        clfcap1 = LocalOutlierFactor(n_neighbors=20, novelty=True, contamination=0.1)
        clfcap1.fit(X_traincap)
        evaluateur.append([clf1,clfcap1])
        nomevaluateur.append("LOF distance n =" + str( round(i+10,1) ) + "contamination = " + str(round(0.01*j,2)))
    ...
    while ntrameRMC < 1000:
        ...
        if message.sentence_type == "RMC" and ntrameRMC >= departdiff:
            ...
            for i in range(0, len(evaluateur)):
                scoring = testeval(evaluateur[i][0],evaluateur[i][1], X_test, X_testcap,brouilvrai, brouilvraicap)
