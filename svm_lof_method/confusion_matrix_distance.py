...
# Ce programme utilise les données de RMC_45min_dataset.json comme données d'entrainement (modifiez le chemin d'accés de RMC_45min_dataset.json ligne 170)
...
for kernel in listetypekernel:
    ...
    for i in range(1,10):
        for j in range(1,10):
            print(i)
            clf = svm.OneClassSVM(nu=0.01*i, kernel=kernel, gamma=0.1*j)
            clf.fit(X_train)
            clfcap = svm.OneClassSVM(nu=0.007, kernel="rbf", gamma=0.5)
            clfcap.fit(X_traincap)
            evaluateur.append([clf, clfcap])
            nomevaluateur.append("SVM distance type : " + kernel + " nu = " + str(round( 0.001*i, 3)) + " gamma = " + str(round(0.1*j, 2)))
for i in range(20):
    for j in range(1,20):
