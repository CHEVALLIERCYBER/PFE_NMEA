...
#------------------------------------------------------------------------
# @Titre : Score matrix heading
#------------------------------------------------------------------------
# @Description :
#This code is used to determine the best estimator to classify heading data
...
def testeval (est_dist, est_head, X_dist, X_head, true_spoof, scaler) :
    ...
    if brouildeteccap == true_spoof:
        score_sentence = 1
    else:
        score_sentence = 0
    if brouildeteccap and not true_spoof:
        miss = 1
    else:
        miss = 0
...
for kernel in listetypekernel:
    ...
    for i in range(1,10):
        for j in range(1,10):
            print(i)
            clf_distance = svm.OneClassSVM(nu=0.007, kernel="rbf", gamma=0.7)
            clf_distance.fit(X_train_distance)
            clf_heading = svm.OneClassSVM(nu=0.01*i, kernel=kernel, gamma=0.1*j)
...
for i in range(len(score_estimator)):
    ...
    print(estimator_name[i] + " - " + str(score_estimator[i]*100/(RMC_sentence_number-2)) + "% for " + str(miss_estimator[i]*100/(RMC_sentence_number-2)) + "% positive miss")
